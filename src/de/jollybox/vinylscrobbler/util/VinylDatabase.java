...
public class VinylDatabase extends SQLiteOpenHelper {
	...
	private static final int THUMB_STORE_LIMIT = 500;
	...
	public static synchronized VinylDatabase getInstance(Context context) {
		if(instance == null) instance = new VinylDatabase(context);
		return instance;
	}
	...
	public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL("CREATE TABLE discogs_collection ("
				...
				+ "country TEXT," + "thumb_uri TEXT);");
		db.execSQL("CREATE TABLE thumbs ("
				+ "thumb_uri TEXT," + "thumb BLOB,"+"time INTEGER NOT NULL);");
	}

	@Override
	public void onUpgrade(SQLiteDatabase db, int oldversion, int newversion) {
		if (oldversion == 1 && newversion == 2) {
			// add support for discogs collection & thumb storage
			db.execSQL("CREATE TABLE discogs_collection ("
					+ "release_spec TEXT UNIQUE NOT NULL," + "id INT,"
					+ "is_master INT NOT NULL," + "title TEXT NOT NULL,"
					+ "artist TEXT," + "format TEXT," + "label TEXT,"
	...
	public void updateDiscogsCollection(List<ReleaseSummary> discogs) {
		// a map of ids to remove, with their corresponding thumb_uri, to clear the stored thumb
		SparseArray<String> toRemove = new SparseArray<String>();
		// a map of release id keys and index values of the input list for efficient lookups
		...
		curs.close();
		// diff lists have been created, remove local only releases and clear the stored thumb if it exists
	...
	public void storeThumb(String thumbURI, Bitmap thumb) {
		if (thumb != null) {
			...
			//limit number of thumbs to the maximum capacity
			String[] arg = { Integer.toString(THUMB_STORE_LIMIT) };
			Cursor del_curs = db.rawQuery(
					"SELECT thumb_uri FROM thumbs ORDER BY time DESC LIMIT -1 OFFSET ?", arg);
			if (del_curs.moveToFirst()) {
				do {
					arg[0] = del_curs.getString(0);
					db.delete("thumbs", "thumb_uri=?", arg);
				} while (del_curs.moveToNext());
			}
			del_curs.close();
		}
